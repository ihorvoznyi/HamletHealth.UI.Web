...
import { CoffeeSvg, FootballSvg, FriendsSvg, GiftSvg, MovieSvg, ShoppingSvg } from '@components/ui/svg';
  ...
  return (
    <div className={classes.container}>
      ...
      <div className={classes.moods}>
        {moods.map(mood => <Fragment key={mood}>{renderMoodSvg(mood)}</Fragment>)}
      </div>

      <ul className={classes.activities}>
        {activities.map(activity => <Activity key={activity.text} {...activity} />)}
      </ul>
    </div>
  );
};
